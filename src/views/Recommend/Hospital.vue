...
<script>
import { SearchHospitalAsync } from "@/api/Hospital";
...
export default {
  ...
  methods: {
    handleFilter() {
      ...
      SearchHospitalAsync(t.query).then(
        response => {
    ...
    checked() {
      ...
      t.grid.list.forEach(item => {
        if (t.cValue.includes(item.hospitalGuid)) {
