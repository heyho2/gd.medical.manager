...
<script>
import { SearchCourseAsync } from "@/api/Course";
...
export default {
  ...
  methods: {
    handleFilter() {
      ...
      SearchCourseAsync(t.query).then(
        response => {
    ...
    checked() {
      ...
      t.grid.list.forEach(item => {
        if (t.cValue.includes(item.courseGuid)) {
