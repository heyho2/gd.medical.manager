...
<script>
import { SearchProductAsync } from "@/api/Product";
...
export default {
  ...
  methods: {
    handleFilter() {
      ...
      SearchProductAsync(t.query).then(
        response => {
    ...
    checked() {
      ...
      t.grid.list.forEach(item => {
        if (t.cValue.includes(item.productGuid)) {
