...
<script>
import { SearchOfficeAsync } from "@/api/Hospital";
...
export default {
  ...
  methods: {
    handleFilter() {
      ...
      SearchOfficeAsync(t.query).then(
        response => {
    ...
    checked() {
      ...
      t.grid.list.forEach(item => {
        if (t.cValue.includes(item.officeGuid)) {
